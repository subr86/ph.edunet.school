 ...
 * @return string
 */
function theme_edunet_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_edunet_set_headerimg($theme);

    $scss .= theme_edunet_set_topfooterimg($theme);

    $scss .= theme_edunet_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_edunet_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_edunet_set_topfooterimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_edunet_set_loginbgimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_edunet_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_edunet', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_edunet and not theme_boost (see the line above).
 ...
 * @return string
 */
function theme_edunet_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_edunet_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
 ...
 * @return string
 */
function theme_edunet_get_setting($setting, $format = false) {
    $theme = theme_config::load('moove');
 ...
 */
function theme_edunet_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_edunet_add_certificatesmenuitem($flatnav);

    theme_edunet_delete_menuitems($flatnav);

    theme_edunet_add_coursesections_to_navigation($flatnav);
}
 ...
 *
 */
function theme_edunet_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_edunet_has_certificates_plugin()) {
            return;
        }
        ...
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_edunet'),
                'shorttext' => get_string('certificates', 'theme_edunet'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_edunet_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_edunet_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_edunet'),
            'shorttext' => get_string('coursesections', 'theme_edunet'),
 ...
 * @return bool
 */
function theme_edunet_has_certificates_plugin() {
